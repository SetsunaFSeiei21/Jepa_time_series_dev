# FEDformer 的 JEPA-style Backbone 重构说明

## 1. 基本信息

- 模型名称：FEDformer
- 代码位置：`src/models/time-series/fedformer.py`
- 建议说明文件位置：`model_update_message/fedformer.md`
- 本次处理目标：只对 FEDformer 的 backbone / forward 接口进行 JEPA-style 重构，不修改训练脚本、不修改 dataloader、不修改 ModelFactory。

本次重构的核心目标是让 FEDformer 同时支持两种训练模式：
...
## 2. 原始 FEDformer 的输入输出约定

当前项目中的 FEDformer 不是直接使用 `(B, N, T)` 格式，而是使用现有 dataloader 的格式：
...
2. 对输入序列做 FEDformer / Autoformer-style decomposition，得到 seasonal / trend 分量；
...
结论：FEDformer 可以合理拆分为 `encode / predict / decode` 三个阶段，但本次建议作为 v0 baseline 使用。

原因如下：

1. FEDformer 有明确的 encoder 路径：`enc_embedding + encoder`；
2. FEDformer 有明确的 decoder 路径：`dec_embedding + decoder + final projection/slicing`；
3. encoder 输出的 hidden representation 形状稳定，可以作为 JEPA 的 embedding representation；
4. FEDformer 原本没有显式 JEPA predictor，但 `Ex` 和 `Ey` 的 shape 可以可靠确定，因此可以添加一个轻量 predictor；
5. 需要注意：FEDformer 的 Fourier / Wavelet attention 模块在初始化时包含静态序列长度假设，因此当前版本优先适用于 `his_len == pred_len` 的实验设置。
...
这里的 `Ex` 和 `Ey` 不是最终值域预测结果，而是 FEDformer encoder 输出的 hidden representation。
...
(B, T, N)     # squeeze 后的 FEDformer 内部格式
...
`encode` 直接复用 FEDformer 原始 encoder 路径：
...
FEDformer 的 encoder 使用 FourierBlock 或 MultiWaveletTransform。当前代码中 FourierBlock 初始化时使用：

```python
seq_len=self.his_len
```

因此本次 `encode(label)` 在当前实验 `his_len == pred_len` 时最稳妥。
...
则需要重新检查 FEDformer 的 label encoding 是否仍然适配，或者为 label 单独构造 target encoder。
...
FEDformer 原本没有显式 predictor，但其 encoder 输出 shape 是可靠的：
...
这里 hidden/channel 维度都是 `d_model`，因此本版本暂时不做 hidden dimension 映射，只做时间维映射：
...
1. learnable future query + TransformerDecoder predictor；
2. FEDformer-style hidden decoder predictor；
3. frequency-domain predictor；
4. AutoCorrelation / FourierCrossAttention 风格的 representation predictor。
...
虽然 `decode` 的核心输入是 `Ey_pred`，但 FEDformer decoder 还需要原始输入序列构造 seasonal/trend 初始化，因此 `decode` 还需要：
...
这个输出 shape 应与原始 FEDformer forward 的输出 shape 保持一致。
...
本次只处理 FEDformer backbone，不需要修改以下文件：

```text
ModelFactory
configs/model/fedformer.yaml
...
1. `ModelFactory` 已经能实例化 FEDformer，不需要改变构造方式；
2. `configs/model/fedformer.yaml` 已经提供 FEDformer 所需参数；
...
本轮 FEDformer 重构只需要修改两个文件：

```text
src/models/time-series/fedformer.py
model_update_message/fedformer.md
```

其中：

- `src/models/time-series/fedformer.py` 是真正的代码修改；
- `model_update_message/fedformer.md` 是本说明文档，用于记录修改原因、shape 约定和人工确认事项。
...
FEDformer 原始 decoder 中使用 historical encoder output `enc_out` 作为 cross representation。JEPA-style 重构后，finetune 路径中 decoder 使用的是 predictor 生成的 `Ey_pred` 作为 cross representation。
...
此外，FEDformer 的 Fourier / Wavelet attention 模块存在静态序列长度假设，因此当前版本优先用于 `his_len == pred_len` 的 setting。如果后续 `his_len != pred_len`，需要人工重新检查 label encoder 和 decoder cross-attention 是否仍然适配。
...
```bash
python -m py_compile src/models/time-series/fedformer.py
```

### 11.2 查看修改内容

```bash
git diff src/models/time-series/fedformer.py
git diff model_update_message/fedformer.md
```

### 11.3 提交修改

```bash
git add src/models/time-series/fedformer.py model_update_message/fedformer.md
git commit -m "Refactor FEDformer backbone for JEPA-style pretraining"
git push
```
...
文件位置：`src/models/time-series/fedformer.py`
...
4. 新增 `_build_decoder_inputs(...)`，封装 FEDformer decoder 所需的 seasonal/trend 初始化；
...
1. 虽然 shape 和模块边界合理，但 decoder 使用 `Ey_pred` 作为 cross representation 后的训练稳定性和最终预测性能需要实验验证；
2. FEDformer 的 Fourier / Wavelet attention 模块存在静态序列长度假设，当前版本优先适用于 `his_len == pred_len` 的实验设置。
